...
namespace ABCRetails.Services
{
    public class FunctionApiClient : IFunctionApi
    {
        ...
        public async Task AddToCartAsync(string customerId, string productId, int quantity)
        {
            ...
            var response = await _http.PostAsJsonAsync("api/cart/add", payload);
        ...
        {
            var response = await _http.GetAsync($"api/cart/{customerId}");
            response.EnsureSuccessStatusCode();
        ...
        {
            var response = await _http.DeleteAsync($"api/cart/{customerId}/{productId}");
            response.EnsureSuccessStatusCode();
        }
        ...
        {
            var response = await _http.DeleteAsync($"api/cart/clear/{customerId}");
            response.EnsureSuccessStatusCode();
        }
